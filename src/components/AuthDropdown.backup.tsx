...
export function AuthDropdown({ user, onLogin, onLogout }: AuthDropdownProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // For demo purposes, allow login with any credentials if Supabase is not configured
      // You can remove this in production when your Supabase is fully configured
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      // For demo purposes, allow signup with any credentials if Supabase is not configured
      // You can remove this in production when your Supabase is fully configured
      if (!process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.NODE_ENV === 'development') {
        // Demo signup (simulated)
        setTimeout(() => {
          const userData = {
